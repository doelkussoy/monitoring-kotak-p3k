...
<body>
    <nav class="navbar glass-navbar fixed-top py-2">
        <div class="container-fluid px-4">
            <div class="d-flex align-items-center gap-3">
                <a href="dashboard.php" class="logo-wrapper bg-accent-light text-accent p-2 rounded-4 shadow-sm text-decoration-none">
                ...
                <a href="dashboard.php" class="d-flex align-items-center gap-3 text-decoration-none">
                    ...
                    <div class="brand-text border-start ps-3 d-none d-sm-block">
                        <h1 class="h6 mb-0 fw-bold tracking-tight text-primary"><?= strtoupper(htmlspecialchars($lokasi['nama_lokasi'])) ?></h1>
                        <p class="text-secondary tiny mb-0 fw-medium" style="font-size: 0.6rem; letter-spacing: 0.05em;">PIC: <?= htmlspecialchars($lokasi['pic_nama'] ?? $lokasi['pic']) ?></p>
                    </div>
                </a>
            </div>
            <div class="d-flex align-items-center gap-4">
                <nav class="d-none d-md-flex align-items-center gap-1">
                    <a href="dashboard.php" class="nav-link-premium"><i data-lucide="layout-dashboard" style="width:16px;"></i> Dashboard</a>
                    <?php if ($_SESSION['role'] == 'Admin'): ?>
                        <a href="laporan.php" class="nav-link-premium"><i data-lucide="file-bar-chart" style="width:16px;"></i> Laporan</a>
                        <a href="pengaturan.php" class="nav-link-premium"><i data-lucide="settings" style="width:16px;"></i> Pengaturan</a>
                    <?php endif; ?>
                </nav>
                ...
                <div class="dropdown">
                    <div class="user-profile-trigger d-flex align-items-center gap-2" data-bs-toggle="dropdown" role="button">
                        <div class="avatar-circle bg-accent-light text-accent rounded-circle d-flex align-items-center justify-content-center" style="width: 36px; height: 36px;"><i data-lucide="user" style="width:18px;"></i></div>
                    ...
                    <ul class="dropdown-menu dropdown-menu-end border-0 shadow-lg mt-3 rounded-4 p-2">
                        ...
                        <li><a class="dropdown-item rounded-3 small d-flex align-items-center gap-2 py-2" href="logout.php"><i data-lucide="log-out" style="width:16px;" class="text-danger"></i><span class="fw-medium">Keluar Sistem</span></a></li>
                    </ul>
                </div>
            </div>
        </div>
    ...
    <div class="container py-3" style="margin-top: 80px; padding-bottom: 100px;">
        <?php if (isset($success_msg)): ?> <div class="alert alert-success glass-card border-0 mb-4 fade-in"><i data-lucide="check-circle" class="me-2" style="width:18px;"></i><?= $success_msg ?></div> <?php endif; ?>
        <?php if (isset($error_msg)): ?> <div class="alert alert-danger glass-card border-0 mb-4 fade-in"><i data-lucide="x-circle" class="me-2" style="width:18px;"></i><?= $error_msg ?></div> <?php endif; ?>
            ...
            <div class="col-12">
                <div class="glass-card p-4">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        ...
                        <button class="btn-premium btn-sm" data-bs-toggle="modal" data-bs-target="#addItemModal"><i data-lucide="plus" style="width:16px;"></i> Tambah Item</button>
                    </div>
                                ...
                                <?php while ($item = mysqli_fetch_assoc($items)):
                                    ...
                                    <tr>
                                        <form action="" method="POST">
                                            ...
                                            <td>
                                                ...
                                                <div class="d-flex align-items-center gap-1 mt-1">
                                                    ...
                                                    <input type="date" name="tgl_kadaluarsa" class="form-control form-control-sm border-0 bg-transparent p-0 tiny fw-medium text-secondary" value="<?= $item['tgl_kadaluarsa'] ?>" style="width: auto; font-size: 0.75rem;">
                                                </div>
                                            </td>
                                            <td>
                                                <div class="d-flex align-items-center gap-2">
                                                    <input type="number" name="stok" class="form-control form-control-sm border-0 bg-light-subtle fw-bold text-primary" value="<?= $item['stok'] ?>" min="0" style="width: 70px; padding: 0.25rem 0.5rem;" required>
                                                    <span class="text-secondary small"><?= htmlspecialchars($item['satuan']) ?></span>
                                                </div>
                                            </td>
                                            <td class="mobile-hide">
                                                <div class="d-flex align-items-center gap-2">
                                                    <input type="number" name="min_stok" class="form-control form-control-sm border-0 bg-transparent text-secondary" value="<?= $item['min_stok'] ?>" min="0" style="width: 60px; padding: 0.25rem 0.5rem;" required>
                                                    <span class="text-secondary small"><?= htmlspecialchars($item['satuan']) ?></span>
                                                </div>
                                            </td>
                                            <td>
                                                <?php if ($is_low_stock): ?> <span class="badge-status badge-critical"><i data-lucide="alert-triangle" style="width:12px;"></i> Rendah</span>
                                                <?php elseif ($is_expired): ?> <span class="badge-status badge-warning"><i data-lucide="clock" style="width:12px;"></i> Kadaluarsa</span>
                                                <?php else: ?> <span class="badge-status badge-success"><i data-lucide="check" style="width:12px;"></i> Aman</span>
                                                <?php endif; ?>
                                            </td>
                                            <td class="text-end">
                                                <div class="d-flex justify-content-end align-items-center gap-1">
                                                    <button type="submit" name="adjust_stock" class="btn btn-sm btn-accent-light text-accent rounded-3 p-2" title="Simpan Perubahan"><i data-lucide="check-circle" style="width:18px;"></i></button>
                                        </form>
                                        <form id="deleteItemForm<?= $item['id'] ?>" action="" method="POST" class="mb-0">
                                            ...
                                            <button type="button" onclick="confirmDeleteItem(<?= $item['id'] ?>, '<?= htmlspecialchars($item['nama_item']) ?>')" class="btn btn-sm btn-outline-danger border-0 rounded-3 p-2"><i data-lucide="trash-2" style="width:18px;"></i></button>
                                        </form>
                                                </div>
                                            </td>
                                    </tr>
            ...
            <div class="modal-content glass-card border-0">
                ...
                <form action="" method="POST">
                    ...
                    <div class="modal-body p-4">
                        <div class="mb-3"><label class="form-label small fw-bold">Nama Item</label><input type="text" name="nama_item" class="form-control" placeholder="Contoh: Kasa" required></div>
                        <div class="row g-3">
                            <div class="col-md-6"><label class="form-label small fw-bold">Stok Awal</label><input type="number" name="stok" class="form-control" value="0" min="0" required></div>
                            <div class="col-md-6"><label class="form-label small fw-bold">Min. Stok</label><input type="number" name="min_stok" class="form-control" value="0" min="0" required></div>
                        </div>
                        <div class="mb-3 mt-3"><label class="form-label small fw-bold">Satuan</label><input type="text" name="satuan" class="form-control" placeholder="pcs, botol, box" required></div>
                        <div class="mb-0"><label class="form-label small fw-bold">Tanggal Kadaluarsa</label><input type="date" name="tgl_kadaluarsa" class="form-control" required></div>
                    </div>
                    <div class="modal-footer border-0 pt-0"><button type="button" class="btn btn-light rounded-3" data-bs-toggle="modal">Batal</button><button type="submit" name="add_item" class="btn-premium">Simpan Item</button></div>
                </form>
            </div>
        </div>
    </div>
