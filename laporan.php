...
<body>
            ...
            <div class="d-flex align-items-center gap-4">
                <nav class="d-none d-md-flex align-items-center gap-1">
                    ...
                    <?php if ($_SESSION['role'] == 'Admin'): ?>
                        <a href="laporan.php" class="nav-link-premium active"><i data-lucide="file-bar-chart" style="width:16px;"></i> Laporan</a>
    ...
    <nav class="mobile-bottom-nav desktop-hide shadow-lg">
        ...
        <?php if ($_SESSION['role'] == 'Admin'): ?>
            <a href="laporan.php" class="mobile-nav-item active"><i data-lucide="file-bar-chart"></i><span>Laporan</span></a>
    ...
    <div class="container py-3" style="margin-top: 100px; padding-bottom: 100px;">
        ...
        <div class="d-flex justify-content-between align-items-end mb-4">
            <div>
                ...
                <p class="text-secondary small mb-0">Ringkasan status ketersediaan dan masa kadaluarsa seluruh unit P3K.</p>
            </div>
            <button onclick="window.print()" class="btn btn-outline-primary rounded-3"><i data-lucide="printer" class="me-2" style="width:16px;"></i>Cetak Laporan</button>
        </div>
                    ...
                    <tbody>
                        <?php while ($row = mysqli_fetch_assoc($q_all_locations)): ?>
                            <tr>
                                ...
                                <td><div class="d-flex align-items-center gap-2"><i data-lucide="package" class="text-secondary" style="width:14px;"></i><span class="fw-medium"><?= (int)$row['total_items'] ?> Item</span></div></td>
                                <td><span class="<?= $row['critical_items'] > 0 ? 'text-danger fw-bold' : 'text-secondary' ?>"><?= (int)$row['critical_items'] ?> Item</span></td>
                                <td><?php if ($row['critical_items'] > 0): ?> <span class="badge-status badge-critical"><i data-lucide="alert-circle" style="width:12px;"></i> Butuh Tindakan</span>
                                    <?php else: ?> <span class="badge-status badge-success"><i data-lucide="check-circle" style="width:12px;"></i> Aman</span>
                                    <?php endif; ?>
                                </td>
                                <td class="text-end"><div class="text-secondary small"><?= date('d/m/Y', strtotime($row['last_update'])) ?><div class="tiny" style="font-size: 0.7rem;"><?= date('H:i', strtotime($row['last_update'])) ?> WIB</div></div></td>
                            </tr>
                        <?php endwhile; ?>
                    </tbody>
                </table>
